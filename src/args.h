...
class ArgumentParsingError : public std::exception {
  ...
  public:
    explicit ArgumentParsingError(std::string msg) { this->msg = std::move(msg); }
...
class Parser {
        ...
        for (int i = 0; i < argc; i++) {
            ...
            case 0:
                ...
                if (std::find(not_positional_args.begin(), not_positional_args.end(), i) != not_positional_args.end())
                    continue;
                if (positional_arg == options.position)
                    return i;
                positional_arg++;
                break;
            case 1:
                ...
                if (!is_bool && options.short_name == argument[size_of_argument - 1]) {
                    if (argc > i + 1) {
                        ...
                        throw Exceptions::ArgumentParsingError((std::string) "Argument " + argument +
                                                               " needs a value. Got none.");
                } else {
                    for (int j = 0; j < size_of_argument; j++)
                        if (argument[j] == options.short_name)
                            return i;
                }
                break;
            case 2:
  ...
  public:
    ...
    template <typename ValueType> ValueType get_arg(const ArgOpt &options) {
        ValueType output;
        bool val_type_is_bool = std::is_same_v<ValueType, bool>;
        int position = get_string(options, val_type_is_bool);
        if (position == -1) {
            std::string s;
            if (options.long_name.empty())
                s += options.short_name;
            else
                s += options.long_name;
    ...
    template <typename ValueType> ValueType get_arg(const ArgOpt &options, const ValueType &default_) {
        ValueType v;
        try {
    ...
    template <typename ValueType> ValueType convert_arg(char *arg) {
...
template <> inline std::string Args::Parser::get_arg<std::string>(const ArgOpt &options) {
...
// converters, yaaay
template <> inline long Args::Parser::convert_arg<long>(char *arg) { return strtol(arg, nullptr, 10); }

template <> inline double Args::Parser::convert_arg<double>(char *arg) { return strtod(arg, nullptr); }
